...
module('scheduler', function () {
  ...
  test('it runs scheduled tasks in the correct order', async function (assert) {
    ...
    scheduler.schedule('sync', () => {
      assert.strictEqual(callCount++, 0, 'sequence is correct');
    });

    scheduler.schedule('layout', () => {
      assert.strictEqual(callCount++, 1, 'sequence is correct');
    });

    scheduler.schedule('measure', () => {
      assert.strictEqual(callCount++, 2, 'sequence is correct');
    });

    scheduler.schedule('affect', () => {
      assert.strictEqual(callCount++, 3, 'sequence is correct');
    });

    scheduler.schedule('affect', () => {
      assert.strictEqual(callCount++, 4, 'sequence is correct');
    });
  ...
  test('it schedules another flush if jobs have been pushed during a flush', async function (assert) {
    ...
    requestAnimationFrame(() => {
      assert.strictEqual(callCount, 1, 'call count is correct after first RAF');
    });
